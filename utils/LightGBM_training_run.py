...
def load_and_prepare_data():
    ...
    TRAINING_WEEKS = 50  # Use 50 weeks of data
    ...
    # Drop columns that cause issues with LightGBM (date columns and other non-numeric)
    features_to_drop = [
        'id',  # UNIQUE_ID_COLUMN
        'grade',  # TARGET_COLUMN
        'snapshot_date', 
...
def main():
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Starting ULTRA-FAST LightGBM training...")
    ...
    try:
        ...
        run_name = f"lightgbm_baseline_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
