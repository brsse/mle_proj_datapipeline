...
def main():
    """
    Main function to execute a standalone model training and tuning run.
    """
    print("--- Starting Standalone Training Run ---")
    
    # 1. Initialize Spark with more memory for the driver
    spark = SparkSession.builder \
        .appName("StandaloneTraining") \
    ...
    # 4. Create a baseline XGBoost multiclass model
    print("\n--- Training Baseline XGBoost Multiclass Model ---")
    ...
    run_id = save_model_with_mlflow(baseline_model, X_train, y_train, grade_mapping, metrics, "baseline_credit_scoring")
    ...
    metrics_data = {
        'run_id': run_id,
        'model_name': 'baseline_credit_scoring',
    ...
    metrics_file = f"/opt/airflow/model_bank/baseline_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
