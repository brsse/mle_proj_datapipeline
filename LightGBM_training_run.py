...
EXPERIMENT_NAME = "credit_scoring_pipeline_lightgbm"
...
def main():
    """
    Main function to execute a standalone model training and tuning run using LightGBM.
    """
    print("--- Starting Standalone Training Run with LightGBM ---")
    
    # 1. Initialize Spark with more memory for the driver
    spark = SparkSession.builder \
        .appName("StandaloneTrainingLightGBM") \
    ...
    # 4. Create a baseline LightGBM multiclass model
    print("\n--- Training Baseline LightGBM Multiclass Model ---")
    ...
    run_id = save_model_with_mlflow(baseline_model, X_train, y_train, grade_mapping, metrics, "baseline_credit_scoring_lightgbm")
    ...
    metrics_data = {
        'run_id': run_id,
        'model_name': 'baseline_credit_scoring_lightgbm',
    ...
    metrics_file = f"/opt/airflow/model_bank/lightgbm_baseline_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
